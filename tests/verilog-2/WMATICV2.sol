...
contract WMATICV2 is IWMATICV2, ReentrancyGuard {
    ...
    */
    ...
    function getBounty() internal {
        uint256 delta = _totalSupply >= _balanceOfMatic
            ? _totalSupply - _balanceOfMatic
            : _balanceOfMatic - _totalSupply;

        uint256 tolerance = _balanceOfMatic / 10;
        if (delta > tolerance) {
            bug();
        }
    }
