    ...
    I: Input,
{
    ...
    fn get_is_state_used(&self, state: &mut S, idx: usize) -> i64;
}
...
struct VoteData {
    pub used: bool,
    pub storage_candiates: Vec<usize>,
    pub storage_map: HashMap<usize, usize>,
    pub prev_sorted_votes: Vec<usize>,
    ...
    I: Input + Debug,
{
    ...
    fn on_add(&self, state: &mut S, idx: usize) -> Result<(), Error> {
        ...
        if !state.has_metadata::<VoteData>() {
            ...
            let mut candidates: Vec<usize> = Vec::with_capacity(DROP_THRESHOLD);
            for i in 0..DROP_THRESHOLD+1 {
                candidates.push(i);
            }
            state.metadata_mut().insert(VoteData {
                used: false,
                storage_candiates: candidates,
                storage_map: HashMap::new(),
                votes_and_visits: HashMap::new(),
                sorted_votes: vec![],
                visits_total: 1,
                votes_total: 1,
                prev_sorted_votes: vec![],
            });
        }
        ...
        let mut to_remove: Vec<usize> = vec![];
        {
            ...
            // println!("SCHEDULER => to removed(); Size = {:?}", corpus_size);
    ...
    I: Input,
{
    ...
    // fn get_sorted_votes(&self, state: &mut S) -> Vec<usize> {
    //     let mut changed = vec![];
    //     let data = state.metadata_mut().get_mut::<VoteData>().unwrap();
    //     // println!("sorted_vote length == {:?}", data.sorted_votes.len());
    //     if data.sorted_votes != data.prev_sorted_votes {
    //         for i in 0..data.sorted_votes.len() {
    //             if data.sorted_votes.get(i) != data.prev_sorted_votes.get(i) {
    //                 changed.push(i);
    //                 // println!("different!!! at {:?}", i);
    //             } else {
    //                 // println!("same!!! at {:?}", i);
    //             }

    //         }
    //         data.prev_sorted_votes = data.sorted_votes.clone();
    //     } 
    //     // data.sorted_votes.clone()
    //     changed
    // }
