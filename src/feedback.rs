...
use crate::evm::host::{BRANCH_DISTANCE_INTERESTING, BRANCH_DISTANCE_CHANGED};
    ...
    Out: Default,
{
    ...
    where
        ...
        OT: ObserversTuple<I, S>,
    {
        // println!("dataflow is_interesting");
...
/// for fuzzing.
///
pub static mut DF_MAP: [u8; MAP_SIZE] = [0; MAP_SIZE];
#[cfg(feature = "cmp")]
    ...
    Out: Default,
{
    ...
    {
        // println!("compare  is_interesting");
        let mut cmp_interesting = false;
        let cov_interesting = false;
        unsafe { BRANCH_DISTANCE_CHANGED = false; }

        // check if the current distance is smaller than the min_map
        for i in 0..MAP_SIZE {
            // distance += self.current_map[i].
            // if unsafe { !BRANCH_DISTANCE_CHANGED } && self.current_map[i] != self.min_map[i] {
            //     unsafe { BRANCH_DISTANCE_CHANGED = true; }
            // }
        ...
        if cmp_interesting {
            ...
            println!("less distance in CPU");
        }
        unsafe { BRANCH_DISTANCE_INTERESTING = cmp_interesting; }

        // if coverage has increased, vote for the state
        if cov_interesting {
            self.scheduler
                .vote(state.get_infant_state_state(), input.get_state_idx());
            println!("new coverage in CPU");
        }
        ...
        unsafe {
            ...
            if self.vm.deref().borrow_mut().state_changed() || pc_interesting {
                ...
                for i in 0..MAP_SIZE {
                    // if DF_MAP[i] == 0 && cur_read_map[i] && cur_write_map[i] != 0 {
                ...
                // println!("[Debug] df_interesting = {:?}; pc_interesting = {:?}", df_interesting, pc_interesting);
