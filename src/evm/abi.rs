...
use crate::evm::abi::BasicVarType::{*};
...
pub trait ABI: CloneABI + serde_traitobject::Serialize + serde_traitobject::Deserialize {
    ...
    /// get the basic types of each 32 bytes
...
impl BoxedABI {
    ...
    pub fn get_bytes(&self) -> Vec<u8> {
        // let a = self.b.get_bytes();
        // println!("abi-> get_bytes sum {:?} => {:02x?}", self.to_string(), a);
        // println!("abi-> get_bytes flags => {:?}", self.b.get_basic_types());
        [Vec::from(self.function), self.b.get_bytes()].concat()
    }

    /// Get the encoded args (transaction data)
    ...
    Addr: Clone + Debug + Serialize + DeserializeOwned,
{
    ...
    // println!("sample_abi => size = {:?}", size);
    if size == 32 {
        // sample a static type
        // println!("sample_abi => matchinging = {:?}", state.rand_mut().below(100));
...
impl ABI for A256 {
    ...
    fn get_basic_types(&self) -> Vec<BasicVarType> {
        ...
        if self.is_address {
            ...
            let t : BasicVarType = unsafe { 
                let src : u8 = TvStrBytes1 as u8 + data_len.as_u8() - 1;
                std::mem::transmute(src)
            };
            vec![t]
        } else {
            let t : BasicVarType = unsafe { 
                let src : u8 = TvInt8 as u8 + data_len.as_u8() - 1;
                std::mem::transmute(src)
            };
            vec![t]
        }
    ...
    fn get_bytes(&self) -> Vec<u8> {
        ...
        // println!("abi-> get_bytes A256 => {:?}", bytes);
        bytes
    }
    ...
    fn set_bytes(&mut self, bytes: Vec<u8>) {
        let offset = 32 - self.data.len();
        self.data = bytes[offset..32].to_vec();
    }
...
impl ABI for ADynamic {
    ...
    fn get_basic_types(&self) -> Vec<BasicVarType> {
        let mut types : Vec<BasicVarType> = vec![TvLength];
        for _ in 0..(self.data.len() / self.multiplier) {
            types.push(TvStrBytes32);
        }
        let remaining = self.data.len() % self.multiplier;
        if remaining != 0 {
            let t : BasicVarType = unsafe { 
                let src : u8 = TvStrBytes1 as u8 + remaining.as_u8() - 1;
                std::mem::transmute(src)
            };
            types.push(t);
        }
        ...
        // [vec![TvLength], vec![TvStrBytes32; new_len / 32]].concat()
    }
    ...
    fn get_bytes(&self) -> Vec<u8> {
        ...
        // println!("abi-> get_bytes dynamic => {:?}", bytes);
        bytes
    }
...
impl ABI for AArray {
    ...
    fn get_basic_types(&self) -> Vec<BasicVarType> {
        // for flags
        let mut flag_tail_data: Vec<Vec<BasicVarType>> = Vec::new();
        let mut flag_head: Vec<Vec<BasicVarType>> = Vec::new();
        let mut flag_head_data: Vec<Vec<BasicVarType>> = Vec::new();
        ...
        for i in 0..self.data.len() {
            if self.data[i].is_static() {
                ...
                head.push(encoded);
                flag_head.push(self.data[i].get_basic_types());  // 
                tail_data.push(dummy_bytes.clone());
                flag_tail_data.push(vec![TvNone; 0]); //
            } else {
                tail_data.push(self.data[i].get_bytes_vec());
                flag_tail_data.push(self.data[i].get_basic_types()); //
                head.push(dummy_bytes.clone());
                flag_head.push(vec![TvNone; 0]); //
                head_size += 32;
            }
        }
        ...
        if tail_data.len() > 0 {
            ...
            for i in 0..tails_offset.len() {
                if head[i].len() == 0 {
                    ...
                    flag_head_data.push(vec![TvNone; 1]);//
                    head_data_size += 32;
                    set_size(head_data[i].as_mut_ptr(), tails_offset[i] + head_size);
                    flag_head_data[i][0] = TvOffset;
                } else {
                    head_data.push(head[i].clone());
                    flag_head_data.push(flag_head[i].clone());  //
        ...
        let mut flags = vec![TvNone; bytes.len() / 32];
        ...
        for i in 0..head_data.len() {
            ...
            //
            flags[(offset/32)..((offset + head_data[i].len()) / 32)]
                .copy_from_slice(flag_head_data[i].to_vec().as_slice());
        ...
        for i in 0..tail_data.len() {
            ...
            //
            flags[(offset/32)..((offset + tail_data[i].len()) / 32)]
                .copy_from_slice(flag_tail_data[i].to_vec().as_slice());
    ...
    fn get_bytes(&self) -> Vec<u8> {
        ...
        // println!("abi-> get_bytes aarray => {:?}", bytes);
        bytes
    }
    ...
    fn set_bytes(&mut self, bytes: Vec<u8>) {
        ...
        // println!("step3 => item len: {:?}", self.data.len());
        // for item in self.data.iter_mut() {
        //     println!("innerstep {:?}", item.get_type_str());
        //     if item.is_static() {
        //         let len = item.b.get_size();
        //         let mut new_bytes = vec![0; len];
        //         new_bytes.copy_from_slice(&bytes[offset..offset + len]);
        ...
        //         item.b.set_bytes(new_bytes);
        //         println!("offset = {:?}, size = {:?}", offset, len);
        //         offset += len;
        //     } else {
        ...
        //         };
        //         println!("offset = {:?}, size = {:?}", tail_offset, tail_size);
        //         index += 1;
