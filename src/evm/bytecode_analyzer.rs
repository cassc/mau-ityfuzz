    ...
    S: HasMetadata + State,
{
    ...
    // println!("original constants from PUSH_ANY: {:?}", constants);
...
pub fn collect_constants(bytecode: &Bytecode)
{   
    // let constants = find_constants(bytecode);
    // #[cfg(feature = "cuda")]
    // {
    //     println!("constants from PUSH_ANY: {:?}", constants);
    //     #[link(name = "runner")]
    //     extern "C" {
    //         fn cuLoadConstBytes(idx: u32, constant: *const u8, size: u8);
    //     }
    //     let mut idx: u32 = 0;
    //     for constant in constants {
    //         unsafe {
    //             cuLoadConstBytes(idx, constant.as_ptr(), constant.len() as u8);
    //         }
    //         idx += 1;
    //     }
    // }
