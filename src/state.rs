...
use libafl::state::{
    ...
    HasSolutions, State, HasStartTime,
};
...
use crate::evm::config::{GPU_ENABLE};
    ...
    Loc: Clone + Debug + Serialize + DeserializeOwned,
{
    ...
    fn add_infant_state<SC>(
        &mut self, 
        state: &StagedVMState<Loc, Addr, VS>, 
        scheduler: &SC
    ) -> Option<usize>
    where
        SC: Scheduler<StagedVMState<Loc, Addr, VS>, InfantStateState<Loc, Addr, VS>>;
...
where
    VS: Default + VMStateT + 'static,
    VI: VMInputT<VS, Loc, Addr> + Input,
    Addr: Serialize + DeserializeOwned + Clone + Debug + PartialEq + 'static,
    Loc: Serialize + DeserializeOwned + Debug + Clone,
    Out: Default,
{
    ...
    fn add_caller(&mut self, addr: &Addr) {
        if !self.callers_pool.contains(addr) {
            ...
            if unsafe { GPU_ENABLE } {
                #[link(name = "runner")]
                extern "C" {
                    fn cuAddCallerPool(added_caller: *const u8, pool_len: u32);
                }
                let mut cuadr = addr
                    .clone()
                    .as_any()
                    .downcast_ref::<EVMAddress>()
                    .unwrap()
                    .to_fixed_bytes();
                cuadr.reverse();
                unsafe {
                    cuAddCallerPool(cuadr.as_ptr(), self.callers_pool.len() as u32);
                }
            }
        }
    ...
    fn add_address(&mut self, caller: &Addr) {
        if !self.addresses_pool.contains(caller) {
            ...
            if unsafe { GPU_ENABLE } {
                #[link(name = "runner")]
                extern "C" {
                    fn cuAddAddressPool(added_addr: *const u8, pool_len: u32);
                }
                let mut cuadr = caller
                    .clone()
                    .as_any()
                    .downcast_ref::<EVMAddress>()
                    .unwrap()
                ...
                // cuadr.reverse();
                unsafe {
                    cuAddAddressPool(cuadr.as_ptr(), self.addresses_pool.len() as u32);
                }
            }
        }
    }
    ...
    Out: Default,
{
        ...
        SC: Scheduler<StagedVMState<Loc, Addr, VS>, InfantStateState<Loc, Addr, VS>>,
    {
        ...
        // println!("infant states size => {:?}", self.infant_states_state.corpus().count());
        Some(idx)
    }
}
...
impl<VI, VS, Loc, Addr, Out> HasStartTime for FuzzState<VI, VS, Loc, Addr, Out>
where
    VS: Default + VMStateT,
    VI: VMInputT<VS, Loc, Addr> + Input,
    Addr: Serialize + DeserializeOwned + Debug + Clone,
